...
import { MessageCircle, X, Send, Paperclip, Image as ImageIcon, Loader2 } from 'lucide-react';
...
export function Chatbot() {
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            // We need to sanitize history (remove images from history sent to backend if backend doesn't handle them in history param yet, 
            // but our backend expects JSON string. Let's send simplified history)
            ...
            // Use port 8001 as per previous context (backend runs on 8001/8002, let's try 8001 or relative path if proxy is set)
            // Assuming Vite proxy is set up or we use full URL. 
            // Let's try relative path first, assuming proxy. If not, we might need localhost:8001
